...
static bool lowerIntrinsic(CallInst *CI) {
  ...
  case Intrinsic::memset: {
    ...
    FunctionCallee Fn = M->getOrInsertFunction(
        "dilithium_memset", Type::getVoidTy(Ctx), I8PtrTy, I8Ty, I32Ty);
  ...
  case Intrinsic::memcpy: {
    ...
    FunctionCallee Fn = M->getOrInsertFunction(
        "dilithium_memcpy", Type::getVoidTy(Ctx), I8PtrTy, I8PtrTy, I32Ty);
...
std::unique_ptr<Module> c2ir(const std::vector<std::string> &filepaths,
  ...
  auto composite = std::make_unique<Module>("dilithium_composite", llvm_ctx);
...
int main(int argc, char **argv) {
  ...
  for (int dilithiumK = 2; dilithiumK <= 5; dilithiumK++) {
    if(dilithiumK == 4)continue;
    const std::string DILITHIUM_SRC_ROOT = "../../../ref";

    std::vector<std::string> files = {
        DILITHIUM_SRC_ROOT + "/test/test_dilithium.c",
        DILITHIUM_SRC_ROOT + "/randombytes.c",
        DILITHIUM_SRC_ROOT + "/kem.c",
        DILITHIUM_SRC_ROOT + "/indcpa.c",
        DILITHIUM_SRC_ROOT + "/polyvec.c",
        DILITHIUM_SRC_ROOT + "/poly.c",
        DILITHIUM_SRC_ROOT + "/ntt.c",
        DILITHIUM_SRC_ROOT + "/cbd.c",
        DILITHIUM_SRC_ROOT + "/reduce.c",
        DILITHIUM_SRC_ROOT + "/verify.c",
        DILITHIUM_SRC_ROOT + "/fips202.c",
        DILITHIUM_SRC_ROOT + "/symmetric-shake.c",
        "../mem.c"
    };
    ...
    std::vector<std::string> defines = {
        "DILITHIUM_K=" + std::to_string(dilithiumK),
    };
    ...
    int bits = (dilithiumK == 2) ? 2 : (dilithiumK == 3) ? 3 : 5;
    std::string outPath = "../dilithium_IR/dilithium" + std::to_string(bits) + ".ll";
