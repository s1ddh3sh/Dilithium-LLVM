...
void shake128_absorb(keccak_state *state, const uint8_t *in, size_t inlen) {
  unsigned int pos =
      keccak_absorb(*state, (unsigned int)(*state)[SHAKE_STATE_POS],
                    SHAKE128_RATE, in, inlen);
  (*state)[SHAKE_STATE_POS] = pos;
}
...
void shake128_finalize(keccak_state *state) {
  keccak_finalize(*state, (unsigned int)(*state)[SHAKE_STATE_POS],
                  SHAKE128_RATE, 0x1F);
...
void shake128_squeeze(uint8_t *out, size_t outlen, keccak_state *state) {
  unsigned int pos =
      keccak_squeeze(out, outlen, *state,
                     (unsigned int)(*state)[SHAKE_STATE_POS], SHAKE128_RATE);
  (*state)[SHAKE_STATE_POS] = pos;
}
...
void shake256_squeeze(uint8_t *out, size_t outlen, keccak_state *state) {
  unsigned int pos =
      keccak_squeeze(out, outlen, *state,
                     (unsigned int)(*state)[SHAKE_STATE_POS], SHAKE256_RATE);
  (*state)[SHAKE_STATE_POS] = pos;
}
