...
int main(void) {
  ...
  for (i = 0; i < NVECTORS; ++i) {
    ...
    polyw1_pack(buf, &w1[0]);
#if GAMMA2 == (Q - 1) / 32
    for (j = 0; j < N / 2; ++j) {
      tmp[2 * j + 0] = buf[j] & 0xF;
      tmp[2 * j + 1] = buf[j] >> 4;
      if (tmp[2 * j + 0] != w1[0][2 * j + 0] ||
          tmp[2 * j + 1] != w1[0][2 * j + 1])
